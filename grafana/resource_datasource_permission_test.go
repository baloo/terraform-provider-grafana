		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccDatasourcePermissionsCheckExists("grafana_data_source_permission.testPermission", &datasourceID),
					resource.TestCheckResourceAttr("grafana_data_source_permission.testPermission", "permissions.#", "4"),
				),
			},
			{
				Config: testAccExample(t, "resources/grafana_data_source_permission/_acc_resource_remove.tf"),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccDatasourcePermissionsCheckEmpty(&datasourceID),
				),
			},
		},
	})
...
func testAccDatasourcePermissionsCheckEmpty(datasourceID *int64) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		client := testAccProvider.Meta().(*client).gapi
...
func testAccDatasourcePermissionCheckDestroy() resource.TestCheckFunc {
	return func(s *terraform.State) error {
		// you can't really destroy datasource permissions so nothing to check for
		return nil
	}
}
